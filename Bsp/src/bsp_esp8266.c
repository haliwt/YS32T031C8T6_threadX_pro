       ...
       auto_connect_wifi_handler();//InitWifiModule();
          ...
          if(discharge_f){
		     MqttData_Publish_PowerOff_Ref();
              tx_thread_sleep(200);// delay_ms(200);//HAL_Delay(200);
	 ...
	 case 0:
	   if(wifi_connected_success_f ==1 && wifi_app_timer_power_on_f ==0){
		     ...
		     if(discharge_f){
				...
				tx_thread_sleep(200);//Publish_Data_fan_Warning(2);//fan warning 
			   //delay_ms(100);
...
static void Update_Dht11_Totencent_Value(void)
{
    ...
    tx_thread_sleep(200);//delay_ms(200);//HAL_Delay(100);
